...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    ...
    const orderId = data.reference_id || data.external_id;
    const isCompleted = event === "payment_session.completed" || ["PAID", "SETTLED"].includes(data.status);
    const isExpired = event === "payment_session.expired" || data.status === "EXPIRED";
    if (!orderId || (!isCompleted && !isExpired)) {
      response.status(400).json({ error: "Invalid webhook payload." });
      return;
    }

    const paymentStatus = isExpired ? "failed" : "paid";
    const orderStatus = isExpired ? "cancelled" : "preparing";
    const db = getAdminDb();
