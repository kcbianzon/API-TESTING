...
export default function PickupReviewPage({
  ...
  const [xenditSession, setXenditSession] = useState<{
    orderId: string;
    amount: number;
    customerName: string;
    ...
    requireAuth(async () => {
      ...
      try {
        ...
        if (paymentMethod === "xendit" || paymentMethod === "paymongo") {
          ...
          const sessionRes = await fetch(`${paymentApiUrl.replace(/\/$/, "")}/create-session`, {
            method: "POST",
            headers: {
          ...
          if (!sessionRes.ok || !sessionData.componentsSdkKey) {
            throw new Error(sessionData.error || "Xendit payment session creation failed.");
          }
          ...
          setXenditSession({
            ...
            componentsSdkKey: sessionData.componentsSdkKey,
          });
          return;
        }
  ...
  return (
    <div className="min-h-[100dvh] overflow-x-hidden bg-[#050505] text-white">
      ...
      {xenditSession && (
        <XenditPaymentModal
          isOpen
          onClose={() => setXenditSession(null)}
          orderId={xenditSession.orderId}
          amount={xenditSession.amount}
          customerName={xenditSession.customerName}
          componentsSdkKey={xenditSession.componentsSdkKey}
          onPaymentSuccess={async () => {
            const currentUser = auth.currentUser;
            if (currentUser) await clearUserFirestoreCart(currentUser.uid);
            clearCart();
            setXenditSession(null);
            setLocation(`/order-success/${encodeURIComponent(xenditSession.orderId)}`);
          }}
        />
      )}
